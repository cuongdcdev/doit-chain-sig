...
export default function ChallengeDetailPage() {
  ...
  // init   
  useEffect(() => {
    const fetchPrizeBalance = async (address, chain) => {
      const prizeBalanceObject = await getPrizeBalance(address, chain);
      if (prizeBalanceObject?.status !== "success") return 0;
      
      // Handle Dogecoin's different response format
      return chain === "Dogecoin"
        ? parseFloat(prizeBalanceObject.balance.length > 0 ? prizeBalanceObject.balance?.[0].value : "0" )
        : parseFloat(prizeBalanceObject.balance);
    };
    ...
    const init = async () => {
      ...
      try {
        ...
        const depositAddr = await generateAddress(challenge.id, challenge.chain);
        if (depositAddr?.status === "success") {
          setDepositAddr(depositAddr.data.address);
          
          // 3. Fetch prize balance
          const balance = await fetchPrizeBalance(depositAddr.data.address, challenge.chain);
      ...
      setTimeout(async () => {
        ...
        if (challenge.status == "ClosedSuccess" || challenge.status == "ClosedFailed") {
          ...
          if (challenge.chain !== "Dogecoin" && prizeBalance && (prizeBalance >= parseFloat(challenge.prize))) {
            ...
            //NOTE: for dogecoin
            console.log("dogecoin finalize challenge: ", challenge);
            setLoadingFinalize(true);
            let final = await finalizeChallenge(challenge.id);
            ...
            setLoadingFinalize(false);
            console.log("dogecoin finalize challenge", final);
          ...
          <div className="position-fixed bottom-0 end-0 p-3" style={{ zIndex: 9999 }}>
            <div className="toast show" role="alert" aria-live="assertive" aria-atomic="true">
              ...
              <div className="toast-body">
                Checking for prize distribution, please wait, may take up to 30 seconds...
              </div>
            </div>
          </div>
        )}
              ...
              <div className="toast show" role="alert" aria-live="assertive" aria-atomic="true">
                ...
                <div className="toast-body">
                  ...
                  {challengeObj.chain === "Dogecoin" && <span className="text-secondary opacity-75 mt-1" style={{ fontSize: "0.8rem" }}>
                    <i className="bi bi-info-circle me-1"></i>
                    Note: Dogecoin transactions may take up to 10 minutes to showup on the explorer, please wait
                  </span>}
                </div>
              </div>
            </div>
